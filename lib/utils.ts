...
export async function parsePDFFile(file: File) {
    try {
        ...
        try {
            // Prefer pdf document destroy if available
            if (pdfDocument && typeof (pdfDocument as any).destroy === 'function') {
                // some builds expose destroy on the PDFDocumentProxy
                await (pdfDocument as any).destroy();
            } else if (loadingTask && typeof (loadingTask as any).destroy === 'function') {
                // some builds expose destroy on the loadingTask
                (loadingTask as any).destroy();
            } else if (pdfDocument && typeof (pdfDocument as any).cleanup === 'function') {
                // older/newer versions may use cleanup()
                await (pdfDocument as any).cleanup();
            } else if (pdfDocument && typeof (pdfDocument as any).close === 'function') {
                // some variants have close()
                await (pdfDocument as any).close();
            }
        } catch (cleanupErr) {
            // don't fail the main flow because cleanup failed; log for debugging
            console.warn('PDF cleanup failed:', cleanupErr);
        }
